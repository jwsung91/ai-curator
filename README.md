...
데일리·위클리는 `scripts/prompt_policy.py`의 공통 근거·선별·분류 기준을 사용합니다. 관찰과 주간 흐름은 근거가 없으면 생략하고, 기사 수가 많다는 이유로 늘리지 않습니다. 입력별 출처·URL·원문 발행일·수집일을 전달하며, 위클리에는 기존 AI 관찰문을 재입력하지 않습니다.
